...
fun describe(result: TransferResult): String = when (result) {
        is TransferResult.Success -> "Successful Transfer!"
        is TransferResult.Failure -> "Transfer Failed."
    }
...
suspend fun fetchAccounts(): List<Account> {
    ...
    val accounts = mutableListOf(
        Account("101", " Checking", -12.50),
        Account("102", "Savings", 50.0),
        Account("103", "Loan" , 12500.0)
    ) 
    return accounts
}
...
fun main() = runBlocking {
    ...
    // operations here.
   val accountList = fetchAccounts()
    
   val negativeBalance = accountList
       .filter { it.balance < 0 }
       .sumOf { it.balance }
   val sumBalance = accountList.sumOf { it.balance }
   
    println("Negative balance: ${negativeBalance.asCurrency()}")
    println("Total balance: ${sumBalance.asCurrency()}")
